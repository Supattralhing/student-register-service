# Week 01 — A student register that remembers

**The week in one sentence:** your Go API stops making up answers and starts keeping real records in a database.

Start with [lesson 6: set up the database](06-start-the-database.md).

| Lesson | What you will be able to do | Suggested time |
| --- | --- | --- |
| [6. Start the database](06-start-the-database.md) | Start PostgreSQL with a student table and two sample students. | 30–45 minutes |
| [7. Explore with DBeaver](07-explore-with-dbeaver.md) | Open the table, find a student, and read a small SQL instruction. | 30–45 minutes |
| [8. Find a student through Go](08-find-a-student.md) | Make `GET /student?studentId=0231` return a saved student. | Two 30–45 minute sessions |
| [9. Register a student through Go](09-register-a-student.md) | Make `POST /student` save a student and return the saved details. | 45–60 minutes |
...
Lesson 8 includes a complete GET example so this checkout has a usable starting point. Lesson 9 builds on it. The finished [reference file](reference/main.go.txt) is an answer sheet for the teacher. Read each small section with the learner before using the complete answer.
...
`database/init/` also contains `02-more-students.sql`, which adds two more sample students for week 02. It changes nothing about lessons 6–9; there are simply four students in the table instead of two.
...
If you are using your own Gin API, add that line to your existing router and restart Go from **server**. Open the address above instead of double-clicking the HTML file: Go serves the page and API from the same address, so the browser can use both without additional cross-origin settings.
...
For the first pass, focus on the path from request to saved record. The teacher can supply the connection setup and error-handling code, then explain those lines gradually. Keep the Go code in one file for these lessons — week 02 splits it up, once there is enough code to make splitting worthwhile.
...
Next: [week 02 — friends](../week02/README.md). Back to the [course index](../../README.md).
